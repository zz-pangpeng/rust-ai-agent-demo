...
use async_openai::types::chat::{ChatCompletionMessageToolCalls, ChatCompletionRequestAssistantMessageArgs, ChatCompletionRequestSystemMessageArgs, ChatCompletionRequestToolMessageArgs, ChatCompletionRequestUserMessageArgs, CreateChatCompletionRequestArgs};
use tracing::{error, info};
use crate::tools::tool::{get_tools};

pub async fn chat_tool_calculator(model: &str, system: &str, prompt: &str) -> anyhow::Result<String> {
        ...
        if let Some(tool_calls) = message.tool_calls {
            ...
            for tool_call in tool_calls {
                if let ChatCompletionMessageToolCalls::Function(tool_call) = tool_call {
                    ...
                    let result = match tools_map.get(name.as_str()) {
                        Some(tool) => {
